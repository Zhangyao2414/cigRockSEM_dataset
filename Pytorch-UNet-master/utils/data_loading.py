...
# 获取标记图像中唯一的像素值或颜色组合，适用于处理语义分割等任务中的标记图像
        ...
        if is_mask:
            ...
            for i, v in enumerate(mask_values):
                if img.ndim == 2:
                    ...
                    mask[(img == v).all(-1)] = i
            # assert np.unique(mask) == [0, 1], f'wrong change,got {np.unique(mask)} values'
...
class CarvanaDataset(BasicDataset):
    def __init__(self, images_dir, mask_dir, scale=1, transform=None):
        super().__init__(images_dir, mask_dir, scale, mask_suffix='-label', transform=transform)
